...
- [Development guidelines](#development-guidelines)
    - [General GalleonCoin Core](#general-galleoncoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
GalleonCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play GALEs (tGALE)" on the test network, if you
...
GalleonCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/galleoncoin-project/galleoncoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_galleoncoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_galleoncoin --log_level=test_suite
$ valgrind -v --leak-check=full src/galleoncoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_galleoncoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates GALEs (if wallet is enabled).
...
However, in open source software such as GalleonCoin Core, where everyone uses
...
pay attention to for reviewers of GalleonCoin Core code.

General GalleonCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: GalleonCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against GalleonCoin Core so that they can be integrated
...
file descriptors it will cause problems with GalleonCoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:GalleonCoin-Project/GalleonCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `galleoncoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `galleoncoin-cli` and the GUI debug console use this table to determine how to
