...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zGALE stake block is less than allowed start height", __func__);

    if (CZPivStake* zGALE = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zGALE->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zGALE stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zGALE stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zGALE->GetValue()));
        if (nChecksum200 != zGALE->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zGALE->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zGALE fails context checks", __func__);
    } else {
        ...
        CPivStake* galeInput = new CPivStake();
        galeInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(galeInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zGALE is checked above in ContextualCheckZerocoinStake()
