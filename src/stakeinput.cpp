...
// The zGALE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zGALE is a hash of the serial
...
{
    //Create an output returning the zGALE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZGALEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zGALE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zGALE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZGALEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zGALE output", __func__);
...
{
    CzGALETracker* zgaleTracker = pwallet->zgaleTracker.get();
    CMintMeta meta;
    if (!zgaleTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zgaleTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a GALE stake is the outpoint
