...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zgalespends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 GALE at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGALE send
enum ZerocoinSpendStatus {
    ZGALE_SPEND_OKAY = 0,                            // No error
    ZGALE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGALE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGALE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGALE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGALE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGALE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGALE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGALE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGALE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGALE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGALE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGALE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGALE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGALE_SPENT_USED_ZGALE = 14,                      // Coin has already been spend
    ZGALE_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGALE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZGALEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzgaleAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzGALETracker> zgaleTracker;
    ...
    void setZWallet(CzGALEWallet* zwallet)
    {
        zwalletMain = zwallet;
        zgaleTracker = std::unique_ptr<CzGALETracker>(new CzGALETracker(strWalletFile));
    }

    CzGALEWallet* getZWallet() { return zwalletMain; }
    ...
    /** zGALE reset */
    boost::signals2::signal<void()> NotifyzGALEReset;
