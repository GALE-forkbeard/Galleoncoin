...
public:
    ...
    bool WriteZGALESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGALESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGALESeed_deprecated(uint256& seed);
    bool EraseZGALESeed();
    bool EraseZGALESeed_deprecated();

    bool WriteZGALECount(const uint32_t& nCount);
    bool ReadZGALECount(uint32_t& nCount);
