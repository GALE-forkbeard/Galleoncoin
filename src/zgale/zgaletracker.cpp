...
CzGALETracker::CzGALETracker(std::string strWalletFile)
{
...
CzGALETracker::~CzGALETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGALETracker::Init()
{
...
bool CzGALETracker::Archive(CMintMeta& meta)
{
...
bool CzGALETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGALETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGALETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGALETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzGALETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzGALETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzGALETracker::GetSerialHashes()
{
...
CAmount CzGALETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGALETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGALETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGALETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGALETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGALETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGALETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGALETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGALETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGALETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGALETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzGALEWallet* zGALEWallet)
{
    bool iszGALEWalletInitialized = (NULL != zGALEWallet);
    CMintMeta meta;
    ...
    if (! iszGALEWalletInitialized)
        zGALEWallet = new CzGALEWallet(strWalletFile);
    meta.isSeedCorrect = zGALEWallet->CheckSeed(dMint);
    if (! iszGALEWalletInitialized)
        delete zGALEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzGALETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGALETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGALETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGALETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGALETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGALETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzGALEWallet* zGALEWallet = new CzGALEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zGALEWallet);
        }
        delete zGALEWallet;
        LogPrint("zero", "%s: added %d dzgale from DB\n", __func__, listDeterministicDB.size());
    }
