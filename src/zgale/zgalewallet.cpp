...
CzGALEWallet::CzGALEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgale seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGALESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGALESeed_deprecated()) {
                    LogPrintf("%s: Updated zGALE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgale seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgale wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGALEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGALECount(nCountLastUsed);
    else if (!walletdb.ReadZGALECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGALEWallet::Lock()
{
    seedMaster = 0;
}

void CzGALEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGALEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGALE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGALEWallet::LoadMintPoolFromDB()
{
...
void CzGALEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGALEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGALEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgaleTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGALEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGALE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgaleTracker which also adds to database
    pwalletMain->zgaleTracker->Add(dMint, true);
...
void CzGALEWallet::SeedToZGALE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGALEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGALEWallet::GenerateDeterministicZGALE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGALEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGALEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzGALEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
